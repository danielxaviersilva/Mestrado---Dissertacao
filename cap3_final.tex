...
\centering
    \subfloat[Ilustração - processo de \textit{ray-casting}. O plano de visualização da imagem é ortogonal à Fig. \ref{fig::raycasting_3d}. Por simplicidade na imagem, mostramos apenas seis \textit{pixels}. A quantidade real de \textit{pixels} processada para este plano é dado pelo comprimento da imagem em Fig. \ref{fig::raycasting_3d}.] {\includegraphics[width=.57\linewidth, angle=0]{figs/Esquema_Glifo/raycasting/raycasting_2d.png}
    \label{fig::raycasting_2d}
    }
    \hspace{1em}
    \subfloat[Volume 3D renderizado via \textit{ray-casting}] {\includegraphics[width=.35\linewidth, angle=0]{figs/Esquema_Glifo/raycasting/raycasting_3d.png}
    \label{fig::raycasting_3d}
    }
     \caption{Ilustração do processo de \textit{ray-casting} de um volume MRI. Em vermelho, os raios lançados por pixel. Os pontos brancos indicam o ponto de amostragem do raio para com pontos do volume, onde o valor do \textit{voxel} $C_{src}$ e $\alpha_{src}$ são extraídos do volume para cômputo de $C_{dst}$ e $\alpha_{dst}$. Fig. \ref{fig::raycasting_2d} ilustra o \textit{ray-casting} para os \textit{pixels} na metade da visualização 3D em um plano ortogonal à \ref{fig::raycasting_3d}.}
...
\todo[inline]{O seguinte parágrafo surgiu muito abuptamente. \textcolor{green}{OK!}}
