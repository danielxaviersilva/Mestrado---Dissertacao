...
\captionsetup[subfloat]{farskip=0pt,nearskip=0pt}
    ...
    \caption{Ilustração das configurações de coexistência de fibra intravóxel. As cores vermelho e verde ilustram fibras de dois tratos diferentes}
...
\citeonline{popko2012} discute diversas maneiras de aproximar uma esfera a partir de um icosaedro (Fig. \ref{fig::icosaedro}), entre as quais, escolhemos a subdivisão por cordas iguais  (\textit{Class I - Equal-Chords}). Esta subdivisão, além de manter a \textit{quasi}-uniformidade dos pontos na esfera, também herda do icosaedro a propriedade de que todos os pontos têm as suas respectivas antípodas no conjunto de vértices. A manutenção dos pares de antípodas é importante pois exploramos a simetria das dODFs para reduzirmos a quantidade das amostras em uma semi-esfera (Fig. \ref{fig::direcoes}). Mencionamos a subdivisão por arcos iguais (\textit{Class I - Equal Arcs, three great circles}), que além de gozar das mesmas propriedades presentes na subdivisão por cordas iguais e tenha os vértices computados na subdivisão mais uniformemente distribuídos sobre a esfera, apresenta uma complexidade computacional maior.
...
\hline
\end{tabular}
\caption{Esquema de renderização multimodal com glifos de ODF em uma imagem $850\times 850$. Fig. \ref{fig::qualidade_visual_longe_lowres} foi gerado com os parâmetros da última linha da tabela.}
