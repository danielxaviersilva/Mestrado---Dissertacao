...
\citeonline{shattuck2008} mostram uma abordagem utilizando polígonos, que é a mesma deste trabalho, e relatou uma grande limitação na sua performance, sendo assim, não podendo ser utilizado em tempo interativo. A performance relatada pelos autores é de 10 FPS para uma malha esférica de 225 pontos em uma cena no qual há 2 milhões de triângulos renderizados. No trabalho deles, consta que não foi utilizado programação em GPU.

\citeonline{peeters2009} apresentaram um esquema de renderização utilizando \textit{ray casting}, no qual foi aperfeiçoado por \cite{peeters2011}. Esta abordagem possibilitou incrementos em performance em relação a \citeonline{shattuck2008}.
...
A inicialização do esquema consiste no pré-cômputo de uma malha esférica unitária centrada na origem com um conjunto de N pontos $\Pi = \{P_1, P_2, \dots, P_N\}$ e o seu respectivo \mathit{index buffer} e enviá-los à GPU apenas uma vez como um atributo a ser instanciado.

\subsection{Estrutura de dados e tráfego CPU-GPU}
\label{ssec::datastruct}

Dois elementos customizam cada um dos glifos: amostras ODF e sua posição. Esses elementos são enviados para a GPU em cada solicitação de sorteio.

Primeiro, vamos definir o conjunto de vetores $\Upsilon$ como uma função de $\Pi$ como mostrado na Eq.\ref{eq::u_vector_set}. A geometria do glifo é deformada escalonando cada ponto $P_k$ na malha para o valor ODF $\psi (\mathbf{u}_k) $. A matriz de translação coloca a malha esférica deformada para ser centralizada em um ponto $C(x, y, z)$.
...
Devido à grande quantidade de dados enviados à GPU nas solicitações de desenho, o objetivo desta subseção é mostrar uma maneira de estruturar os dados na CPU, como enviá-los à GPU e como os shaders da GPU os lêem para que o tráfego de dados CPU-GPU seja o mínimo possível.

\subsubsection {Translação}
Seja $P$ o número de instâncias do esquema de renderização e glifos a serem desenhados. Os pontos de translação são organizados como um vetor de pontos $[C_1, C_2,\dots, C_P] $, onde $ C_i = (x_i, y_i, z_i)$, e enviados para a GPU como um atributo. Cada ponto deste atributo é único para cada uma das P instâncias da malha esférica.

\subsubsection {ODFs}

Os dados de ODF enviados para a GPU consistem nas N amostras referentes aos perfis de difusão para cada um dos P glifos a serem desenhados. Construímos uma matriz $\mathbf {\Psi}_{PxN} $, onde P é o número de glifos a serem desenhados e N é o número de amostras de ODF usadas, conforme mostrado na equação \ref {eq::Psi}:
...
\begin{bmatrix} 
    ...
    \vdots & \vdots & \vdots & \vdots  \\   
     \psi_P(\mathbf{u}_1)&\psi_P(\mathbf{u}_2) & \cdots \psi_P(\mathbf{u}_{N-1}) & \psi_P(\mathbf{u}_{N})
\end{bmatrix}, 
\endgroup
\end{equation}

onde $\psi_ {ij} $ se refere ao valor ODF $\psi_i (\mathbf {u} _j) $ do i-ésimo glifo associado à direção $\mathbf {u} _j $, que escala $ P_j $ vértice da malha esférica. A matriz $\mathbf {\Psi} _ {PxN} $ é enviada para a GPU como uma textura 2D.

Na GPU, o acesso das amostras de ODF para deformar seu respectivo ponto de esfera é feito acessando os dados de textura por meio de sua instância e índices de vértice no \mathit{vertex shader}.

O índice de instância (Instance\_ID) corresponde a uma malha esférica instanciada centrada em seu respectivo ponto $ P $, e como as amostras ODFs de um glifo são armazenadas nas linhas de $\mathbf {\Psi} $, quando Instance\_ID é usado para acessar esta dimensão para o acesso das amostras de ODF por glifo.

O índice do vértice (Vertex\_ID), por sua vez, indica um ponto da malha esférica, que corresponde à coluna de seu respectivo valor ODF em $\mathbf{\Psi}$.

Assim, realizamos o acesso GPU por um simples \textit{lookup} na textura correspondente a $\mathbf{\Psi}$\footnote{Em OpenGL, o \textit{lookup} é feito através do comando texelFetch($\mathbf {\Psi}$, ivec2 (gl\_InstanceID, gl\_VertexID), 0)[0]} usando o par Vertex\_ID e Instance\_ID. A figura \ref{fig::organizacao2GPU} ilustra o acesso ODF para cada instância e vértice na GPU.

\figura de acesso aqui
...
Em segundo lugar, sugerimos que a estrutura de dados da malha esférica simétrica escolhida seja organizada de forma que um ponto $P $em um índice par seja seguido por $-P$. Esta malha esférica é enviada para a GPU da mesma forma indicada na subseção \ref {ssec::precomputation}.

Seguindo essas recomendações, a estrutura de dados dos pontos na esfera $[P_1, P_2, P_3, P_4, \dots, P_ {N-1}, P_N] $torna-se $[P_1, -P_1, P_3, -P_3, \dots , P_ {N-1}, -P_ {N-1}]$e seu $ \Upsilon$ associado muda para $[\mathbf{u}_1, - \mathbf{u}_1, \mathbf{u}_3, -\mathbf{u}_3, \dots, \mathbf{u}_{N-1}, - \mathbf{u}_{N-1}]$. Nesta situação, $\mathbf{\Psi}$ se torna:
...
\begin{bmatrix} 
    ...
    \vdots & \vdots & \vdots & \vdots  \\
     ...
     \psi_P(\mathbf{u}_1)&\psi_P(-\mathbf{u}_1) & \cdots \psi_P(\mathbf{u}_{N-1}) & \psi_P(-\mathbf{u}_{N-1})
...
Portanto, definimos uma matriz $\mathbf{\Psi^h}_{Px\frac{N}{2}}$de tal forma que $\mathbf{\psi^h}_{ij} = \mathbf{\psi}_{i(2j-1)}$, descrita na expressão abaixo.
...
\begin{bmatrix} 
    ...
    \vdots & \vdots & \vdots & \vdots  \\
     ...
     \psi_P(\mathbf{u}_1)&\psi_P(\mathbf{u}_3) & \cdots \psi_P(\mathbf{u}_{N-3}) & \psi_P(\mathbf{u}_{N-1})
...
\centering
\captionsetup[subfloat]{farskip=0pt,nearskip=0pt}
    \subfloat[Esquema de renderização para ODFs genéricas ]{\includegraphics[width=.75\linewidth, angle=0]{figs/Esquema_Glifo/benchmark_full.png}
    \label{fig::benchmark_full}
    }
    %\hfill
    \\
    \subfloat[Esquema de renderização otimizado para ODFs simétricas ]{\includegraphics[width=.75\linewidth, angle=0]{figs/Esquema_Glifo/benchmark_half.png}
...
O algoritmo iterativo de integração de linhas foi baseado no proposto por \citeonline{Weinstein1999}, enquanto os critérios de parada do processo iterativo foi implementado de acordo com \citeonline{basser2000}.
...
%Deixar o usuário a par do que é anisotropia e direções.
\begin{itemize}
    ...
    \end{itemize}
    \item parâmetro de advecção $w_{punct}$ (definido por \citeonline{Weinstein1999})
\end{itemize}.
...
\citeonline{Chamberland2016} propôs um algoritmo inovador de tractografia em tempo real baseada em picos e que serviu de base para este trabalho.

A sua abordagem de tractografia consiste em um processo iterativo para usar um conjunto de direções de fibras pré-computados a partir de métodos HARDI. Outras abordagens de tractografia HARDI, como a de \citeonline{descoteaux2007}, por exemplo, computa ODFs e extrai a distribuição de fibras durante o processo iterativo.
...
Em trabalhos na área, é muito comum a discretização de uma malha esférica via pontos obtidos através da subdivisão de um icosaedro, devido a uma certa distribuição uniforme no conjunto de pontos sobre a esfera e a sua formulação analítica. Em seu algoritmo de tractografia proposto, \citeonline{descoteaux2007} utiliza uma malha esférica relativa a uma subdivisão de $16^a$ ordem de um icosaedro, de 2562 pontos. Já \citeonline{yeh2010} discretiza !!Falar do icosaedro desses caras.
...
Na tractografia, os conjuntos de direções de fibras e o GFA são pré-computadas para cada amostra do DWI. O pré-cômputo das direções para referência na construção dos \textit{streamlines} é o que torna o método viável para cômputo em tempo real \cite{Chamberland2016}.%A função ExtractLocalMaxSet (linha \ref{alg::extractLocalMaxSet}), como proposto por \citeonline{Chamberland2016}, referencia o conjunto de máximos locais vem de um conjunto pré-computado para cada amostra, assim como o ocorre também com o GFA (linha \ref{alg::GFA}).
