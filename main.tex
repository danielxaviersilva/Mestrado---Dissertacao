...
\item[PGSE] \textit{Pulsed Gradient Spin Echo} \textcolor{red}{(Eco de \textit{spin} do gradiente pulsado)}
...
\item[VMTK-Neuro] \textit{Visual Manipulation Toolkit for NeuroImages} \textcolor{red}{(\textit{Kit} de Ferramentas de Manipulação Visual para Neuroimagens)}
...
A visualização do comportamento de difusão por amostra facilita a análise local dos sinais escaneados, mas o objetivo final da maioria das aplicações são os feixes de fibras que conectam de forma plausível as amostras locais de difusão meio de tractografia. As fibras reconstruídas podem ser representadas por linhas, como ilustrado na Fig. \ref{fig::intro_ex_DTI_tractografia}. Algoritmos de tractografia baseados em DTI foram propostos, por exemplo, por \citeonline{Weinstein1999} e \citeonline{basser2000} e são categorizados como baseados no rastreamento de direções de difusão (\textit{streamline fiber tractography}). Esses algoritmos consistem essencialmente em conectar as amostras pelas direções dos autovetores de maior autovalor de tensores de difusão locais até um critério de parada\sout{, relacionado à chegada da linha em regiões fora da substância branca,}  ser satisfeito. Informações adicionais sobre as diferentes abordagens em tractografia podem ser encontradas em \cite{tournier2011, DTI_Handbook}.

\begin{figure}[ht]
\captionsetup[subfloat]{farskip=0pt,nearskip=0pt}
    \subfloat[Tensores de difusão representado por glifos superquádricos em uma fatia axial do cérebro na região do corpo caloso. As cores representam a direção predominante de difusão onde vermelho é \sout{lateral (}esquerda-direita\sout{)}, azul é \sout{ascendente (}superior-inferior\sout{)} e verde é anteroposterior \sout{(anterior-posterior)}]{\includegraphics[width=.52\linewidth, angle=0]{figs/Introducao/ex_DTI_Superquadrica.png}
...
As ODFs associam uma direção, representada pelo vetor unitário $\mathbf{\hat{u}}$, a um valor escalar de probabilidade $\psi(\mathbf{\hat{u}})$ representativo. Entre os métodos de alta ordem, há os que visam computar funções de distribuição de orientações de difusão (dODF) \cite{TuchQBall2004, wedeen2005, yeh2010}. A partir das dODFs, a informação sobre as fibras subjacentes intravóxel é extraída de um conjunto de direções \sout{feita}\textcolor{red}{obtidas}  através do cômputo dos máximos locais das funções. Se o cômputo das dODFs é feito a partir de aquisições com uma razão sinal-ruído \todo{Quanto é razoável?}razoável, as direções das fibras subjacentes tendem a ser bem modeladas através de seus máximos locais \cite{fillard2011}.
...
\sout{Fig. \ref{fig::flowchart_vmtk_hardi} contextualiza a contribuição esperada do presente trabalho à arquitetura do VMTK-Neuro.} Na Fig. \textcolor{red}{\ref{fig::flowchart_vmtk_hardi}} destacamos os blocos que devem ser integrados ao VMTK-Neuro para visualização das funções ODFs em glifos e tractografia: 
...
Este trabalho aborda a implementação de um algoritmo de renderização de glifos a partir dos dados de ODFs\textcolor{red}{, detacado em ciano na Fig. \ref{fig::flowchart_vmtk_hardi}}. Escolhemos a Amostragem Generalizada no Espaço-Q (\textit{Generalized Q-Sampling Imaging}, GQI) para a síntese dos sinais de volumes de ressonância magnética ponderada em difusão em ODFs. 
...
O VMTK-Neuro dispõe de uma série de funções para explorar as difusões intravóxeis por meio de glifos superquádricos \textcolor{red}{e} as plausíveis conexões intervóxeis por meio de tratos reconstruídos. 
...
A relação da Eq. \ref{eq::stanner} é válida para $D$ somente em difusão livre (Fig. \ref{fig::intro_difusao_isotropica}), pois para meios que apresentam obstáculos (Fig. \ref{fig::intro_difusao_anisotropica}), a mobilidade molecular não é a mesma para todas as direções \cite{douek1991}, \sout{sendo}\textcolor{red}{variando} assim\sout{,} o valor do ADC \sout{varia} com a orientação $\mathbf{\hat{q}}$ do gradiente de ponderação de difusão aplicado.
...
A informação referente ao processo de difusão é extraída dos três autovetores e autovalores da matriz $\mathbf{D}$ (Eq. \ref{eq::tensor}), onde a direção da fibra é usualmente paralela \textcolor{red}{à} direção do autovetor associado ao maior autovalor.
...
Técnicas de misturas assumem que o sinal de difusão medido para um \textit{voxel} é a soma dos sinais gerados por cada população de fibra presente nele \cite{tournier2011}. Como exemplos, há a abordagem multi-tensor, também presente na tese de doutorado de \citeonline{tuch2002}, e a abordagem por deconvolução esférica, proposta \sout{originalmente} por \citeonline{Tournier2004DirectEO}.
...
Nesta seção faremos uma breve apresentação do método GQI (\textit{Generalized Q-sampling Imaging}), a sua fundamentação teórica, na qual mostraremos a relação entre o sinal de difusão $W(\mathbf{r},\mathbf{q})$\textcolor{red}{,} e a função de distribuição de \textit{spins} (\textit{spin distribution function}, SDF) por uma multiplicação matricial. Mostraremos uma formulação discreta de SDFs e a sua relação linear com dODFs. Ambas diferem 
...
\end{equation}
onde $\text{sinc}(x) = \text{sin}(x)/x$ para todo $x$ real, exceto \textcolor{red}{em $x=0$}\sout{ e}\textcolor{red}{. Por definição, } $\text{sinc}(0) = 1$.
...
Observe que a distância de difusão \textcolor{red}{se} ajusta \textcolor{red}{à} contribuição de cada $W(\mathbf{r}, \mathbf{q})$ no cômputo de $\psi_m(\mathbf{r}, \mathbf{\hat{u}})$ e funciona como um parâmetro de regularização. Se $L_{\Delta}$ tende a infinito, a função de distribuição irá se comportar como um impulso na origem, gerando um $\psi_m(\mathbf{r}, \mathbf{\hat{u}})$ fortemente ponderado pelas direções de $\mathbf{q}$ ortogonais a $\mathbf{\hat{u}}$. Um $L_{\Delta}$ pequeno implica em uma maior ponderação para coeficientes que não são ortogonais a $\mathbf{\hat{u}}$, o que atua como uma filtragem. No caso extremo, $L_{\Delta} = 0$, \sout{implica que} todos os pesos aplicados para cômputo de ODFs em todas as direções $\mathbf{\hat{u}}$ se tornem iguais.

Considerando que a distribuição da difusão seja Gaussiana, a distância de difusão é dada por $L_{\Delta}=(6D\lambda)^{1/2}$, onde $D$ é o coeficiente de difusão da água na temperatura do corpo e $\lambda = \Delta - \delta/3$ que\textcolor{red}{, por sua vez,} correspondem aos parâmetros de aquisição mostrados em Fig. \ref{fig::pgse_ilustrado}. 
...
A estimativa \sout{da SDF}\textcolor{red}{de uma função de distribuição de \textit{spins}, SDF,} por \textit{voxel}, 
...
\sout{é}\textcolor{red}{pode ser} feita por meio de $S$ amostras, nas quais são reconstruídas num domínio de $S$ direções de interesse $\mathbf{U} = [
\mathbf{\hat{u}}_1, 
\mathbf{\hat{u}}_2, 
...
\citeonline{popko2012} discute diversas maneiras de aproximar uma esfera a partir de um icosaedro (Fig. \ref{fig::icosaedro}), entre as quais, escolhemos a subdivisão por cordas iguais  (\textit{Class I - Equal-Chords}). Esta subdivisão, além de manter a \textit{quasi}-uniformidade dos pontos na esfera, também herda do icosaedro a propriedade de que todos os pontos têm as suas respectivas antípodas no conjunto de vértices. A manutenção dos pares de antípodas é importante pois exploramos a simetria das dODFs para \sout{armazenarmos}\textcolor{red}{reduzirmos a quantidade d}as amostras em uma semi-esfera (Fig. \ref{fig::direcoes}). \sout{Mencionamos}\textcolor{red}{Embora} a subdivisão por arcos iguais (\textit{Class I - Equal Arcs, three great circles})\sout{, a qual}, além de gozar das mesmas propriedades presentes na subdivisão por cordas iguais, \textcolor{red}{tenha} os vértices computados na subdivisão \sout{são} mais uniformemente distribuídos \sout{na}\textcolor{red}{sobre a} esfera, \sout{a custo de}\textcolor{red}{apresenta} uma complexidade computacional maior.
...
\begin{figure}[H]
    ...
    \caption{ \sout{Ilustração da} Subdivisão por cordas iguais. Vértices do icosaedro destacados por círculos vermelhos.
    }
...
%}
 ...
 \citeonline{Kindlmann2004} propõe glifos superquádricos para representar tensores de segunda ordem. Os autovalores são mapeados na forma de um glifo, enquanto os autovetores em orientações espaciais\todo{Maepamento de cor é adicional.}\sout{ e cores} dos glifos, como ilustra Fig. \ref{fig::intro_ex_DTI_glifos}. Ressaltamos que na Fig. \ref{fig::intro_ex_DTI_glifos}, os eixos x, y e z da cena são coerentes com a direção horizontal, vertical e normal aos planos anatômicos\sout{ e as}\textcolor{red}{. É comum agregar as} cores vermelho, verde e azul \textcolor{red}{para reforçar} \sout{reforçam} a informação da dominância das componentes x, y e z na orientação dos glifos\todo{Citar este trabalho: https://www.researchgate.net/publication/12456615\_Color\_Schemes\_to\_Represent\_the\_Orientation\_of\_Anisotropic\_Tissues\_From\_Diffusion\_Tensor\_Data\_Application\_to\_White\_Matter\_Fiber\_Tract\_Mapping}~\cite{???}. Estes glifos permitem sintetizar numa única imagem não só o comportamento local (intravóxel) de difusão como também as relações entre \textit{voxels} (intervóxel), quando glifos referentes aos \textit{voxels} vizinhos são renderizados lado a lado.
...
%    %!!VER SE ISSO TA CERTO
     \subfloat[16 glifos ODF destacados em laranja na Fig. \ref{fig::QBall_glifos_coronal_fatorpequeno}.] {\includegraphics[width=.4\linewidth, angle=0]{figs/Exemplos_QBall_visualizacao/Coronal_zoom.png}
     \label{fig::QBall_glifos_coronal_fatorgrande}
     }    
     \hfill
     \subfloat[Fatia Coronal.]{\includegraphics[width=.7\linewidth, angle=0]{figs/Exemplos_QBall_visualizacao/Coronal.png}
     \label{fig::QBall_glifos_coronal_fatorpequeno}
     }
      \caption{Glifos ODF sobrepostos aos seus respectivos \textit{voxels} em uma fatia coronal de um MRI T1 anatômico no ambiente de visualização multimodal de ressonância magnética VMTK-Neuro. O volume é artificial e usado no ISMRM \textit{Challenge} 2015 \cite{TractometerTool}.}
     %\hfill
     \label{fig::QBall_glifos_coronal}
 \end{figure}
...
Encorajamos experimentar usar os glifos ODF para escolha das sementes \textcolor{red}{dos feixes de fibras nervosas de interesse}. 
...
Neste apêndice, apresentamos os dados de desempenho relativos à série de experimentos para renderização de glifos para diferentes ordens de tesselação do icosaedro, conforme descrita na Seção \ref{sec::funcionalidades_agregadas_e_ganhos_de_performance}\sout{ do Capítulo \ref{sec::experimentos}}. Foram utilizadas as tesselações de ordem 2, 4, 8 e 16 do icosaedro para o cômputo do tempo de renderização dos glifos.

A primeira série de experimentos se refere à funcionalidade de indexação e síntese do glifo na CPU, a segunda série se refere à adição da \sout{funcionalidade de} instanciação na renderização e a síntese do glifo na GPU, a terceira série se refere à exploração da simetria das ODFs para diminuição do uso de memória e tráfego de dados CPU-GPU dos escalares que sintetizam \sout{a} ODF\textcolor{red}{s} e o quarto experimento se refere ao \sout{uso}\textcolor{red}{acesso coalescente} da memória da GPU\sout{ em um leiaute tal que os acessos dos escalares de ODF que sintetizam os glifos ocorrem de forma coalescente}.
...
\hline
\end{tabular}
\caption{Ambiente de visualização multimodal com glifos ODF renderizados em uma imagem $850\times 850$ com \textcolor{red}{um}a malha esférica \sout{base} fixa. Fig. \ref{fig::qualidade_visual_longe_highres} \sout{teve em sua geração,}\textcolor{red}{foi gerada com} os parâmetros da última linha da tabela.
}
...
\hline
\end{tabular}
\caption{Esquema de renderização multimodal com glifos de ODF \sout{renderizados} em uma imagem $850\times 850$. Fig. \ref{fig::qualidade_visual_longe_lowres} \sout{teve em sua geração,}\textcolor{red}{foi gerado com} os parâmetros da última linha da tabela.}
