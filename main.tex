...
Este capítulo é a base para as discussões dos capítulos \ref{chap::renderizacao_de_perfis_de_difusao}, em que detalhamos um esquema de renderização para visualização dos perfis de difusão obtidos através destes métodos de imageamento e \ref{chap::tractografia}, no qual discutimos a implementação da tractografia a partir deste método.
...
Algo relevante para o tNo que diz respeito a adequabilidade para tractografia, não há métodos que se sobressaem em relação a outros métodos nas mais diferentes condições experimentais \cite{SCHILLING2019194}.
...
\section{Questões computacionais}

Há duas maneiras de codificar métodos HARDI: a primeira consiste
...
A extração de picos do GQI envolve a comparação de amostras de ODF com suas vizinhas. O critério de vizinhança é definido através da malha esférica utilizada para implementação de glifos, mostrada no capítulo \ref{chap::renderizacao_de_perfis_de_difusao}.
