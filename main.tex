...
\todo[inline]{Aqui você deu novamente um pulo de gato. O que é $\mathbf{\hat{u}}$? \textcolor{green}{Resolvido} E onde foi parar $t$ da Eq. \ref{eq::propagator}? \textcolor{green}{Resolvido. t é constante e o tempo de difusão $\Delta$, saiu do argumento da função e virou subíndice, como no artigo original do DSI e GQI} O texto acima não é claro ...}
\begin{equation}
...
Há diversas maneiras de aproximar uma esfera a partir de um icosaedro, que são discutidas em \citeonline{popko2012}. Para este trabalho, escolhemos a subdivisão \textit{Class I - Equal-Chords}. Esta subdivisão mantém a \textit{quasi}-uniformidade nos pontos da esfera e herda a simetria do icosaedro em relação aos eixos. Mencionamos a subdivisão \textit{Class I - Equal Arcs (three great circles)} \citeonline{popko2012}, no qual, além de gozar das mesmas propriedades presentes em  \textit{Class I - Equal-Chords}, os vértices computados na subdivisão são mais uniformemente distribuídos na esfera \cite{popko2012}. Porém, a subdivisão \textit{Class I - Equal-Chords} é computacionalmente mais simples para cômputo dos vértices da subdivisão.
...
\begin{figure}[htb]
    ...
    \subfloat[Subdivisão dos pontos] {\includegraphics[width=.7\linewidth,  angle=0]{figs/Esquema_Glifo/ico_subdivision.png}
    \label{fig::icosaedro_faces_2}
    }
    \\
     \subfloat[Subdivisão dos triângulos do icosaedro. Os vértices do icosaedro estão contidos nos círculos vermelhos]{\makebox[1.35\width]
    ...
    \caption{ Ilustração da subdivisão \textit{Class I - Equal-Chords} para o icosaedro. Definimos um grid paralelo a cada aresta, no qual é equidistante da aresta e do vértice oposto. Os vértices $v0'$, $v1'$ e $v2'$ consistem na intersecção dos \textit{grids} com as arestas do triângulo. A projeção na esfera da interseção gera $v01$, $v12$ e $v20$, que são adicionados ao conjunto de pontos da subdivisão.
    }
...
\captionsetup[subfloat]{farskip=0pt,nearskip=0pt}
\centering
    \subfloat[Novos vértices computados através da subdivsão de cada aresta em quatro segmentos] {\includegraphics[width=.45\linewidth, angle=0]{figs/HARDI/Subdivisao_icosaedro/divisao_faces_4.png}
...
Com o objetivo de enfatizar as direções dominantes de difusão na renderização de glifos, \citeonline{TuchQBall2004} sugere normalizar cada função de distribuição de \textit{spins} $\boldsymbol{\psi}_m(\mathbf{r})$ pela normalização min-max, que gera uma função de distribuição $\boldsymbol{\psi}(\mathbf{r})$: 
