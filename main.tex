...
Definimos a vizinhança de uma direção $\mathbf{u}$ para uma função esférica, discretizada em um conjunto finito de direções $\mathbf{U}$ como o conjunto $\mathbf{N_u}$, de acordo com o mostrado na equação \ref{eq::neighboorhood}.
...
\label{eq::neighboorhood}
    \mathbf{N_u} = \{\mathbf{s} \in \mathbf{U}  | \mathbf{u}.\mathbf{s} > \cos{\theta_{thr}} \}
\end{equation}
...
A direção $\mathbf{u}$ é considerado máximo local se $\psi(\mathbf{u}) > \psi(\mathbf{s}) \; \forall \; \mathbf{s} \in \mathbf{N_u}$.
