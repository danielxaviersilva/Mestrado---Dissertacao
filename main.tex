...
%Adicionalmente, apresentaremos regiões onde são reconhecidas três configurações de fibras que coexistem anatomicamente: o ``beijo'', a bifurcação e cruzamento de fibras. As imagens geradas são do algoritmo proposto integrado ao ambiente de viualização exploratória multimodal de glifos ODF sobrepostos a um MRI ponderado em T1. Faremos uma analise comparativa entre os glifos ODF e superquádricos do DTI nestas regiões e explicitaremos as limitações do DTI face a métodos de alta ordem através dos glifos.
...
Definindo a probabilidade de deslocamento $\mathbf{u}$ durante um dado tempo de difusão $\Delta$ como propagador de difusão (\textit{ensemble-average diffusion propagator}) $\bar{p}_{\Delta}(\mathbf{r}, \mathbf{u})$, \citeonline{tuch2002} mostrou-se que os sinais de difusão aplicados neste intervalo de tempo, $W_{\Delta}(\mathbf{r}, \mathbf{q})$, guardam com o propagador de difusão a seguinte relação de Fourier 
...
Integramos o algoritmo de renderização de glifos ODF no VMTK-Neuro. Na Tabela \ref{tab::glyph_info_experiment_fixed}, mostramos os dados de desempenho associados ao glifo sintetizado a partir da malha esférica obtida através da 8$^a$ ordem de tesselação do icosaedro e na Tabela \ref{tab::glyph_info_experiment}, habilitamos o cômputo da ocupação máxima $max_p$ em \textit{pixels} em tela da projeção de um \textit{voxel} para seleção da resolução apropriada.
...
\hline
\end{tabular}
\caption{Esquema de renderização multimodal com glifos de ODF em uma imagem $850\times 850$. Fig. \ref{fig::qualidade_visual_longe_lowres} foi gerada com os parâmetros da última linha da tabela.}
