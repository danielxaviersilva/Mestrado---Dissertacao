...
As medições de tempo foram efetuadas pela diferenças na hora do sistema entre dois quadros obtidas pela função \textit{Now()} do cabeçalho \textit{Chrono} da biblioteca padrão do C++. Renderizamos a mesma cena cem vezes, computamos a diferença de tempo entre dois quadros e extraímos o valor médio das medições. As limitações de taxa quadros por segundo na aplicação foram desativadas para medição\footnote{Devido à pandemia e às restrições ao acesso do laboratório de controle e automação da UNICAMP, não efetuei as medições em computadores providos de placa gráfica da NVIDIA no laboratório. Para placas da NVIDIA, há um \textit{software} chamado NSight Studio (\url{https://developer.nvidia.com/nsight-visual-studio-edition}) que permite fazer, com maior acurácia, diversas medidas em relação ao tempo de execução e ao uso de memória}.
...
%\captionsetup[subfloat]{farskip=0pt,nearskip=0pt}
    %\rule{6cm}{3cm}
    \subfloat[Visão coronal do trato cortico-espinhal.]{\makebox[1.0\width]{\includegraphics[width=.35\linewidth, angle=0]{figs/Renderizacao_glifos_evolucao/Tratos/trato_1_tf.png}
    }
   \label{fig::tratos_1}
   }
...
Nesta seção apresentamos algumas imagens representativas, em glifos ODF e em glifos superquádricos. Elas ilustram diferenças no poder informativo dos dois tipos de visualização de difusões intravóxeis quanto aos feixes de fibras subjacentes. Escolhemos seis sujeitos para análise das regiões, os quais nos referimos como Sujeitos 1, 2, 3, 4, 5 e 6. Todos os sujeitos são saudáveis e têm seus sexos e faixas etárias anotados na Tabela \ref{tab::vol_id}. 
...
As fatias coronais mostradas nas Fig.
...
\ref{fig::149741_localizacao} representam a localização das regiões dos cérebros dos sujeitos, de 1 a 6, respectivamente a serem analisadas. Destacamos três regiões para análise em cada fatia, que são a interface entre o cíngulo e o corpo caloso (destacada em verde), a base do leque da coroa radiada (destacada em laranja) e o centro semioval (destacado em ciano), que são mostradas em adição à posição relativa do plano coronal em relação as vistas axiais e sagitais.

Afim de facilitar a análise visual, as fatias selecionadas para o experimento das diversas configurações de fibras ocorrem de forma que as fibras a serem analisadas estejam contidas no plano coronal selecionado ou são normais ao plano.

Adicionalmente, mostramos as regiões destacadas com um maior nível de detalhamento para análise de cada um dos sujeitos em sequência após as imagens das suas localizações na ordem: interface cíngulo e o corpo caloso, base do leque da coroa radiada e cruzamento no centro semioval. Trazemos os glifos ODF do GQI e os superquádricos do DTI para que possamos analisá-los de forma comparativa. 

Para melhorar a interpretabilidade dos glifos nas regiões analisadas, fizemos alguns destaques nas imagens com maior nível de detalhamento das configurações mostradas. Por estarem em uma pequena quantidade em relação a totalidade dos glifos trazidas na imagens com maior detalhamento, destacamos os glifos ODF que evidenciam a transição entre cíngulo e corpo caloso em branco e os glifos que evidenciam a bifurcação na coroa radiada em vermelho escuro para todos os sujeitos. Adicionalmente, para enfatizar o poder informativo dos glifos ODF no cruzamento triplo entre SLF, CC e CST, destacamos um glifo ODF no \textit{voxel} de ocorrência no Sujeito 1 (Fig. \ref{fig::172332_crossing_odf}) e o mostramos de forma mais detalhada nas visões coronal, axial e sagital (Fig. \ref{fig::172332_crossing_odf_adicional}). Por completude e para facilitar e associação visual de superquádricos com os glifos ODF para o mesmo \textit{voxel}, também destacamos os superquádricos dos mesmos \textit{voxels} nas imagens com maior nível de detalhamento.
...
A base de um leque de fibras que passam pela capsula interna na coroa radiada estão detalhadas nas Fig.
...
% \textcolor{magenta}{indicando que a segunda direção de difusão dominante é perpendicular ao plano da fatia.}%Comparando esses glifos com os seus correpondentes glifos superquádricos na Fig. \ref{fig::multimodal_fanning_superquadric}...




%Nos resultados mostrados, são esperados que possamos identificar múltiplas direções de fibras subjacentes através dos glifos, sendo duas para a interface do cíngulo e corpo caloso, duas na coroa radiada e duas ou três no centro semioval.

%Fig.  \ref{fig::172332_localizacao} mostra numa mesma fatia coronal as três regiões das quais extraímos as imagens.
...
%Nesta seção, apresentamos algumas imagens representativas dos glifos ODF em uma fatia coronal nas mesmas regiões do cérebro discutidas na Seção \ref{sssec::comparacao_glifosODF_superquadrico} para evidenciar em mais volumes do mesmo banco de dados acerca da eficácia dos glifos ODF na visualização de fibras multidirecionais intravóxeis. As fatias mostradas nas Fig. \ref{fig::172332_localizacao}, \ref{fig::103818_localizacao}, \ref{fig::105923_localizacao}, \ref{fig::122317_localizacao}, \ref{fig::146129_localizacao} e \ref{fig::149741_localizacao} representam diferentes sujeitos, em que nos referimos como sujeitos 1, 2, 3, 4, 5 e 6, respectivamente. As Fig. \ref{fig::103818_}, \ref{fig::105923}, \ref{fig::122317}, \ref{fig::146129} e \ref{fig::149741} ilustram as regiões de coexistência de tratos \todo{O que você quis expressar?}\textcolor{blue}{em um fator de escala maior} para os sujeitos a serem discutidos de 1 a 5, respectivamente. Todos sujeitos nos quais os volumes se referem são saudáveis, e têm seus sexos e faixas-etárias anotados na Tabela \ref{tab::vol_id}.
...
\subsubsection{Sujeito 1}
\label{sssec::sujeito_1}
\begin{figure}[H]
\centering
%\captionsetup[subfloat]{farskip=0pt,nearskip=0pt}
    %\rule{6cm}{3cm}
    \subfloat[Regiões selecionadas para comparação entre os glifos ODF e superquádricos: base do leque de coroa radiada (linha laranja), interface entre cíngulo e corpo caloso (linha verde), e centro semioval (linha ciano). ]{\makebox[1.0\width]{\includegraphics[width=.97\linewidth, angle=0]{figs/Visual/172332/localizacao/coronal.png}
   \label{fig::172332_coronal}
   }
    }
    \\
    \subfloat[Relativa ao  axial]{\includegraphics[width=.26\linewidth, angle=0]{figs/Visual/172332/localizacao/axial.png}
    \label{fig::172332_axial}
    } \hspace{.1\textwidth}
    \subfloat[Relativa ao sagital] {\includegraphics[width=.435\linewidth, angle=0]{figs/Visual/172332/localizacao/sagital.png}
    \label{fig::172332_sagital}
    }
    \caption{Sujeito 1: localização das regiões selecionadas numa fatia coronal.}
    \label{fig::172332_localizacao}
\end{figure}
...
%\captionsetup[subfloat]{farskip=0pt,nearskip=0pt}
    %\rule{6cm}{3cm}
    \subfloat[Glifos ODF do GQI.]{\includegraphics[width=.47\linewidth, angle=0]{figs/Visual/172332/configuracoes/kissing/glifos_ODF.png}
    \label{fig::172332_kissing_odf}
    }
    \hfill
    \subfloat[Superquádricos.] {\includegraphics[width=.47\linewidth, angle=0]{figs/Visual/172332/configuracoes/kissing/superquadricos.png}
    \label{fig::172332_kissing_sqc}
    }
    \caption{Sujeito 1: ``beijo'' na interface entre cíngulo e corpo caloso. As imagens consistem na região destacada em verde na fatia coronal da Fig. \ref{fig::172332_localizacao}.
    }
...
%\captionsetup[subfloat]{farskip=0pt,nearskip=0pt}
    %\rule{6cm}{3cm}
    \subfloat[Glifos ODF do GQI.]{\includegraphics[width=.47\linewidth, angle=0]{figs/Visual/172332/configuracoes/fanning/glifos_ODF.png}
    \label{fig::172332_fanning_odf}
    }
    \hfill
    \subfloat[Superquádricos.] {\includegraphics[width=.475\linewidth, angle=0]{figs/Visual/172332/configuracoes/fanning/superquadricos.png}
    \label{fig::172332_fanning_sqc}
    }
    \caption{Sujeito 1: bifurcação de fibras no trato cortico-espinhal. As imagens consistem na região destacada em laranja na fatia coronal da Fig. \ref{fig::172332_localizacao}.
    }
...
%\captionsetup[subfloat]{farskip=0pt,nearskip=0pt}
    %\rule{6cm}{3cm}
    \subfloat[Glifos ODF do GQI.]{\includegraphics[width=.47\linewidth, angle=0]{figs/Visual/172332/configuracoes/crossing/glifos_ODF}
    \label{fig::172332_crossing_odf}
    }
    \hfill
    \subfloat[Superquádricos.] {\includegraphics[width=.465\linewidth, angle=0]{figs/Visual/172332/configuracoes/crossing/superquadricos}
    \label{fig::172332_crossing_sqc}
    }
    \caption{Sujeito 1: cruzamento no centro semioval numa fatia coronal. As imagens consistem na região destacada em ciano na fatia coronal da Fig. \ref{fig::172332_localizacao}.
    }
...
%\captionsetup[subfloat]{farskip=0pt,nearskip=0pt}
    ...
    \label{fig::172332_crossing_odf_adicional_coronal}
    }
    \hfill
    ...
    \label{fig::172332_crossing_odf_adicional_axial}
    }
    \hfill
    ...
    \label{fig::172332_crossing_odf_adicional_sagital}
    }
    \caption{Detalhes do glifo, destacado com a linha preta na Fig. \ref{fig::172332_crossing_odf}, num \textit{voxel} onde ocorre o cruzamento entre CS (azul, ascendente inclinado para esquerda), CC (azul, ascendente inclinado para direita)  SLF (anteroposterior, verde).}
    \label{fig::172332_crossing_odf_adicional}
\end{figure}

\subsubsection{Sujeito 2}
\label{sssec::sujeito_2}
...
%\captionsetup[subfloat]{farskip=0pt,nearskip=0pt}
    %\rule{6cm}{3cm}
    \subfloat[Regiões selecionadas para comparação entre os glifos ODF e superquádricos: base do leque de coroa radiada (linha laranja), interface entre cíngulo e corpo caloso (linha verde), e centro semioval (linha ciano). ]{\makebox[1.0\width]{\includegraphics[width=.97\linewidth, angle=0]{figs/Visual/103818/localizacao/coronal.png}
   \label{fig::103818_coronal}
   }
    }
    \\
    \subfloat[Relativa ao  axial]{\includegraphics[width=.26\linewidth, angle=0]{figs/Visual/103818/localizacao/axial.png}
    \label{fig::103818_axial}
    } \hspace{.1\textwidth}
    \subfloat[Relativa ao sagital] {\includegraphics[width=.408\linewidth, angle=0]{figs/Visual/103818/localizacao/sagital.png}
    \label{fig::103818_sagital}
    }
    \caption{Sujeito 2: localização das regiões selecionadas numa fatia coronal.}
...
%\captionsetup[subfloat]{farskip=0pt,nearskip=0pt}
    %\rule{6cm}{3cm}
    \subfloat[Glifos ODF do GQI.]{\includegraphics[width=.47\linewidth, angle=0]{figs/Visual/103818/configuracoes/kissing/glifos_ODF}
    \label{fig::103818_kissing_odf}
    }
    \hfill
    \subfloat[Superquádricos.] {\includegraphics[width=.47\linewidth, angle=0]{figs/Visual/103818/configuracoes/kissing/superquadricos}
    \label{fig::103818_kissing_sqc}
    }
    \caption{Sujeito 2: ``beijo'' na interface entre cíngulo e corpo caloso. As imagens consistem na região destacada em verde na fatia coronal da Fig. \ref{fig::103818_localizacao}.
    }
...
%\captionsetup[subfloat]{farskip=0pt,nearskip=0pt}
    %\rule{6cm}{3cm}
    \subfloat[Glifos ODF do GQI.]{\includegraphics[width=.47\linewidth, angle=0]{figs/Visual/103818/configuracoes/crossing/glifos_ODF}
    \label{fig::103818_crossing_odf}
    }
    \hfill
    \subfloat[Superquádricos.] {\includegraphics[width=.47\linewidth, angle=0]{figs/Visual/103818/configuracoes/crossing/superquadricos}
    \label{fig::103818_crossing_sqc}
    }
    \caption{Sujeito 2: cruzamento no centro semioval numa fatia coronal. As imagens consistem na região destacada em ciano na fatia coronal da Fig. \ref{fig::103818_localizacao}.
    }
...
%\captionsetup[subfloat]{farskip=0pt,nearskip=0pt}
    %\rule{6cm}{3cm}
    \subfloat[Glifos ODF do GQI.]{\includegraphics[width=.47\linewidth, angle=0]{figs/Visual/103818/configuracoes/fanning/glifos_ODF}
    \label{fig::103818_fanning_odf}
    }
    \hfill
    \subfloat[Superquádricos.] {\includegraphics[width=.472\linewidth, angle=0]{figs/Visual/103818/configuracoes/fanning/superquadricos}
    \label{fig::103818_fanning_sqc}
    }
    \caption{Sujeito 2: bifurcação de fibras no trato cortico-espinhal. As imagens consistem na região destacada em laranja na fatia coronal da Fig. \ref{fig::103818_localizacao}.
    }
    \label{fig::103818_fanning}
\end{figure}

\subsubsection{Sujeito 3}
\label{sssec::sujeito_3}
%%105923%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[H]
\centering
%\captionsetup[subfloat]{farskip=0pt,nearskip=0pt}
    %\rule{6cm}{3cm}
    \subfloat[Regiões selecionadas para comparação entre os glifos ODF e superquádricos: base do leque de coroa radiada (linha laranja), interface entre cíngulo e corpo caloso (linha verde), e centro semioval (linha ciano). ]{\makebox[1.0\width]{\includegraphics[width=.97\linewidth, angle=0]{figs/Visual/105923/localizacao/coronal.png}
   \label{fig::105923_coronal}
   }
    }
    \\

    \subfloat[Relativa ao  axial]{\includegraphics[width=.26\linewidth, angle=0]{figs/Visual/105923/localizacao/axial.png}
    \label{fig::105923_axial}
    } \hspace{.1\textwidth}
    \subfloat[Relativa ao sagital] {\includegraphics[width=.40\linewidth, angle=0]{figs/Visual/105923/localizacao/sagital.png}
    \label{fig::105923_sagital}
    }
    \caption{Sujeito 3: localização das regiões selecionadas numa fatia coronal.}
...
%\captionsetup[subfloat]{farskip=0pt,nearskip=0pt}
    %\rule{6cm}{3cm}
    \subfloat[Glifos ODF do GQI.]{\includegraphics[width=.47\linewidth, angle=0]{figs/Visual/105923/configuracoes/kissing/glifos_ODF}
    \label{fig::105923_kissing_odf}
    }
    \hfill
    \subfloat[Superquádricos.] {\includegraphics[width=.47\linewidth, angle=0]{figs/Visual/105923/configuracoes/kissing/superquadricos}
    \label{fig::105923_kissing_sqc}
    }
    \caption{Sujeito 3: ``beijo'' na interface entre cíngulo e corpo caloso. As imagens consistem na região destacada em verde na fatia coronal da Fig. \ref{fig::105923_localizacao}.
    }
...
%\captionsetup[subfloat]{farskip=0pt,nearskip=0pt}
    %\rule{6cm}{3cm}
    \subfloat[Glifos ODF do GQI.]{\includegraphics[width=.47\linewidth, angle=0]{figs/Visual/105923/configuracoes/crossing/glifos_ODF}
    \label{fig::105923_crossing_odf}
    }
    \hfill
    \subfloat[Superquádricos.] {\includegraphics[width=.47\linewidth, angle=0]{figs/Visual/105923/configuracoes/crossing/superquadricos}
    \label{fig::105923_crossing_sqc}
    }
    \caption{Sujeito 3: cruzamento no centro semioval numa fatia coronal. As imagens consistem na região destacada em ciano na fatia coronal da Fig. \ref{fig::105923_localizacao}.
    }
...
%\captionsetup[subfloat]{farskip=0pt,nearskip=0pt}
    %\rule{6cm}{3cm}
    \subfloat[Glifos ODF do GQI.]{\includegraphics[width=.47\linewidth, angle=0]{figs/Visual/105923/configuracoes/fanning/glifos_ODF}
    \label{fig::105923_fanning_odf}
    }
    \hfill
    \subfloat[Superquádricos.] {\includegraphics[width=.477\linewidth, angle=0]{figs/Visual/105923/configuracoes/fanning/superquadricos}
    \label{fig::105923_fanning_sqc}
    }
    \caption{Sujeito 3: bifurcação de fibras no trato cortico-espinhal. As imagens consistem na região destacada em laranja na fatia coronal da Fig. \ref{fig::105923_localizacao}.
    }
    \label{fig::105923_fanning}
\end{figure}

\subsubsection{Sujeito 4}
\label{sssec::sujeito_4}
%%122317%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[H]
\centering
%\captionsetup[subfloat]{farskip=0pt,nearskip=0pt}
    %\rule{6cm}{3cm}
    \subfloat[Regiões selecionadas para comparação entre os glifos ODF e superquádricos: base do leque de coroa radiada (linha laranja), interface entre cíngulo e corpo caloso (linha verde), e centro semioval (linha ciano). ]{\makebox[1.0\width]{\includegraphics[width=.97\linewidth, angle=0]{figs/Visual/122317/localizacao/coronal.png}
   \label{fig::122317_coronal}
   }
    }
    \\
    \subfloat[Relativa ao  axial]{\includegraphics[width=.26\linewidth, angle=0]{figs/Visual/122317/localizacao/axial.png}
    \label{fig::122317_axial}
    } \hspace{.1\textwidth}
    \subfloat[Relativa ao sagital] {\includegraphics[width=.44\linewidth, angle=0]{figs/Visual/122317/localizacao/sagital.png}
    \label{fig::122317_sagital}
    }
    \caption{Sujeito 4: localização das regiões selecionadas numa fatia coronal.}
...
%\captionsetup[subfloat]{farskip=0pt,nearskip=0pt}
    %\rule{6cm}{3cm}
    \subfloat[Glifos ODF do GQI.]{\includegraphics[width=.47\linewidth, angle=0]{figs/Visual/122317/configuracoes/kissing/glifos_ODF}
    \label{fig::122317_kissing_odf}
    }
    \hfill
    \subfloat[Superquádricos.] {\includegraphics[width=.462\linewidth, angle=0]{figs/Visual/122317/configuracoes/kissing/superquadricos}
    \label{fig::122317_kissing_sqc}
    }
    \caption{Sujeito 4: ``beijo'' na interface entre cíngulo e corpo caloso. As imagens consistem na região destacada em verde na fatia coronal da Fig. \ref{fig::122317_localizacao}.
    }
...
%\captionsetup[subfloat]{farskip=0pt,nearskip=0pt}
    %\rule{6cm}{3cm}
    \subfloat[Glifos ODF do GQI.]{\includegraphics[width=.47\linewidth, angle=0]{figs/Visual/122317/configuracoes/crossing/glifos_ODF}
    \label{fig::122317_crossing_odf}
    }
    \hfill
    \subfloat[Superquádricos.] {\includegraphics[width=.471\linewidth, angle=0]{figs/Visual/122317/configuracoes/crossing/superquadricos}
    \label{fig::122317_crossing_sqc}
    }
    \caption{Sujeito 4: cruzamento no centro semioval numa fatia coronal. As imagens consistem na região destacada em ciano na fatia coronal da Fig. \ref{fig::122317_localizacao}.
    }
...
%\captionsetup[subfloat]{farskip=0pt,nearskip=0pt}
    %\rule{6cm}{3cm}
    \subfloat[Glifos ODF do GQI.]{\includegraphics[width=.47\linewidth, angle=0]{figs/Visual/122317/configuracoes/fanning/glifos_ODF}
    \label{fig::122317_fanning_odf}
    }
    \hfill
    \subfloat[Superquádricos.] {\includegraphics[width=.471\linewidth, angle=0]{figs/Visual/122317/configuracoes/fanning/superquadricos}
    \label{fig::122317_fanning_sqc}
    }
    \caption{Sujeito 4: bifurcação de fibras no trato cortico-espinhal. As imagens consistem na região destacada em laranja na fatia coronal da Fig. \ref{fig::122317_localizacao}.
    }
    \label{fig::122317_fanning}
\end{figure}

\subsubsection{Sujeito 5}
\label{sssec::sujeito_5}
%%146129%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[H]
\centering
%\captionsetup[subfloat]{farskip=0pt,nearskip=0pt}
    %\rule{6cm}{3cm}
    \subfloat[Regiões selecionadas para comparação entre os glifos ODF e superquádricos: base do leque de coroa radiada (linha laranja), interface entre cíngulo e corpo caloso (linha verde), e centro semioval (linha ciano). ]{\makebox[1.0\width]{\includegraphics[width=.97\linewidth, angle=0]{figs/Visual/146129/localizacao/coronal.png}
   \label{fig::146129_coronal}
   }
    }
    \\
    \subfloat[Relativa ao  axial]{\includegraphics[width=.26\linewidth, angle=0]{figs/Visual/146129/localizacao/axial.png}
    \label{fig::146129_axial}
    } \hspace{.1\textwidth}
    \subfloat[Relativa ao sagital] {\includegraphics[width=.41\linewidth, angle=0]{figs/Visual/146129/localizacao/sagital.png}
    \label{fig::146129_sagital}
    }
    \caption{Sujeito 5: localização das regiões selecionadas numa fatia coronal.}
...
%\captionsetup[subfloat]{farskip=0pt,nearskip=0pt}
    %\rule{6cm}{3cm}
    \subfloat[Glifos ODF do GQI.]{\includegraphics[width=.47\linewidth, angle=0]{figs/Visual/146129/configuracoes/kissing/glifos_ODF}
    \label{fig::146129_kissing_odf}
    }
    \hfill
    \subfloat[Superquádricos.] {\includegraphics[width=.46\linewidth, angle=0]{figs/Visual/146129/configuracoes/kissing/superquadricos}
    \label{fig::146129_kissing_sqc}
    }
    \caption{Sujeito 5: ``beijo'' na interface entre cíngulo e corpo caloso. As imagens consistem na região destacada em verde na fatia coronal da Fig. \ref{fig::146129_localizacao}.
    }
...
%\captionsetup[subfloat]{farskip=0pt,nearskip=0pt}
    %\rule{6cm}{3cm}
    \subfloat[Glifos ODF do GQI.]{\includegraphics[width=.47\linewidth, angle=0]{figs/Visual/146129/configuracoes/crossing/glifos_ODF}
    \label{fig::146129_crossing_odf}
    }
    \hfill
    \subfloat[Superquádricos.] {\includegraphics[width=.458\linewidth, angle=0]{figs/Visual/146129/configuracoes/crossing/superquadricos}
    \label{fig::146129_crossing_sqc}
    }
    \caption{Sujeito 5: cruzamento no centro semioval numa fatia coronal. As imagens consistem na região destacada em ciano na fatia coronal da Fig. \ref{fig::146129_localizacao}.
    }
...
%\captionsetup[subfloat]{farskip=0pt,nearskip=0pt}
    %\rule{6cm}{3cm}
    \subfloat[Glifos ODF do GQI.]{\includegraphics[width=.47\linewidth, angle=0]{figs/Visual/146129/configuracoes/fanning/glifos_ODF}
    \label{fig::146129_fanning_odf}
    }
    \hfill
    \subfloat[Superquádricos.] {\includegraphics[width=.47\linewidth, angle=0]{figs/Visual/146129/configuracoes/fanning/superquadricos}
    \label{fig::146129_fanning_sqc}
    }
    \caption{Sujeito 5: bifurcação de fibras no trato cortico-espinhal. As imagens consistem na região destacada em laranja na fatia coronal da Fig. \ref{fig::146129_localizacao}.
    }
...
%\captionsetup[subfloat]{farskip=0pt,nearskip=0pt}
    %\rule{6cm}{3cm}
    \subfloat[Regiões selecionadas para comparação entre os glifos ODF e superquádricos: base do leque de coroa radiada (linha laranja), interface entre cíngulo e corpo caloso (linha verde), e centro semioval (linha ciano). ]{\makebox[1.0\width]{\includegraphics[width=.97\linewidth, angle=0]{figs/Visual/149741/localizacao/coronal.png}
   \label{fig::149741_coronal}
   }
    }
    \\
    \subfloat[Relativa ao  axial]{\includegraphics[width=.26\linewidth, angle=0]{figs/Visual/149741/localizacao/axial.png}
    \label{fig::149741_axial}
    } \hspace{.1\textwidth}
    \subfloat[Relativa ao sagital] {\includegraphics[width=.37\linewidth, angle=0]{figs/Visual/149741/localizacao/sagital.png}
    \label{fig::149741_sagital}
    }
    \caption{Sujeito 6: localização das regiões selecionadas numa fatia coronal.}
...
%\captionsetup[subfloat]{farskip=0pt,nearskip=0pt}
    %\rule{6cm}{3cm}
    \subfloat[Glifos ODF do GQI.]{\includegraphics[width=.47\linewidth, angle=0]{figs/Visual/149741/configuracoes/kissing/glifos_ODF}
    \label{fig::149741_kissing_odf}
    }
    \hfill
    \subfloat[Superquádricos.] {\includegraphics[width=.475\linewidth, angle=0]{figs/Visual/149741/configuracoes/kissing/superquadricos}
    \label{fig::149741_kissing_sqc}
    }
    \caption{Sujeito 6: ``beijo'' na interface entre cíngulo e corpo caloso. As imagens consistem na região destacada em verde na fatia coronal da Fig. \ref{fig::149741_localizacao}.
    }
...
%\captionsetup[subfloat]{farskip=0pt,nearskip=0pt}
    %\rule{6cm}{3cm}
    \subfloat[Glifos ODF do GQI.]{\includegraphics[width=.47\linewidth, angle=0]{figs/Visual/149741/configuracoes/crossing/glifos_ODF}
    \label{fig::149741_crossing_odf}
    }
    \hfill
    \subfloat[Superquádricos.] {\includegraphics[width=.47\linewidth, angle=0]{figs/Visual/149741/configuracoes/crossing/superquadricos}
    \label{fig::149741_crossing_sqc}
    }
    \caption{Sujeito 6: cruzamento no centro semioval numa fatia coronal. As imagens consistem na região destacada em ciano na fatia coronal da Fig. \ref{fig::149741_localizacao}.
    }
...
%\captionsetup[subfloat]{farskip=0pt,nearskip=0pt}
    %\rule{6cm}{3cm}
    \subfloat[Glifos ODF do GQI.]{\includegraphics[width=.47\linewidth, angle=0]{figs/Visual/149741/configuracoes/fanning/glifos_ODF}
    \label{fig::149741_fanning_odf}
    }
    \hfill
    \subfloat[Superquádricos.] {\includegraphics[width=.465\linewidth, angle=0]{figs/Visual/149741/configuracoes/fanning/superquadricos}
    \label{fig::149741_fanning_sqc}
    }
    \caption{Sujeito 6: bifurcação de fibras no trato cortico-espinhal. As imagens consistem na região destacada em laranja na fatia coronal da Fig. \ref{fig::149741_localizacao}.
    }
...
\ref{fig::122317_kissing},
\ref{fig::146129_kissing} e
\ref{fig::149741_kissing} consistem em transições entre cíngulo e corpo caloso. Observe que, a partir dos glifos superquádricos, não é possível inferir a direção das fibras subjacentes. Esses superquádricos tem forma parecida com alguns glifos dos ventrículos, representado pelo fundo escuro na parte inferior da imagem, onde a difusão é isotrópica. Por outro lado, os glifos ODF contornados pela linha branca, além de destoarem na sua aparência nas formas dos glifos na região isotrópica, eles mostram nitidamente as duas direções associadas ao corpo caloso (vermelho em direção lateral) e ao cíngulo (verde em direção anteroposterior), que são diferentes dos glifos de múltiplas pontas difusas que caracterizam difusões intravóxeis isotrópicas e são facilmente evidenciáveis para os volumes analisados. Por subestimar as direções de difusão intravóxel, os glifos superquádricos podem proporcionar uma noção equivocada das relações intervóxeis e levar a entendimento de conexões incorretas entre regiões corticais. Observe também que os glifos representativos da região do toque entre ambos os tratos ocorrem de forma clara para todos os volumes analisados, o que nos leva a crer que a configuração é facilmente evidenciável para DWIs com a mesma sequência de aquisição e pipeline de pré-processamento do HCP.

% \sout{Adicionalmente, podemos visualizar que há uma correlação intervoxel para os glifos localizados na região de transição, já que ambos podem ser interpretados como uma composição da região acima na imagem (CG) com a de baixo (CC).}


Embora de forma sutil, Fig.
\ref{fig::172332_fanning},
\ref{fig::103818_fanning},
\ref{fig::105923_fanning},
\ref{fig::122317_fanning},
\ref{fig::146129_fanning} e
\ref{fig::149741_fanning} demonstram a capacidade dos glifos ODF em mostrar configurações em ``leque'' (\textit{fanning}), típica do trato cortico-espinhal \cite{fortin2012, wang2019}. Como já destacamos na Seção \ref{ssec::visual_resultados}, os glifos ODF contornados pela linha vermelha escura nas imagens da base do leque da coroa radiada em todos os volumes tem  direções predominantemente ascendentes e levemente inclinadas na direção lateral com uma cor que transita entre azul e magenta, evidenciando mais de uma direção de difusão. Com os glifos superquádricos, não há a possibilidade de inferência sobre a abertura em ``leque'' por serem de formato tubular (Fig. \ref{fig::172332_fanning_sqc} e \ref{fig::146129_fanning_sqc}) ou de paralelepípedo suave (Fig. \ref{fig::103818_fanning_sqc}, \ref{fig::105923_fanning_sqc} e \ref{fig::122317_fanning_sqc}) com tons de cor que variam também entre magenta e azul. Vale ressaltar que há coerência, tanto em cor quanto em forma, entre os glifos ao lado da região de abertura em ``leque'' destacada. A diferença ocorre no meio da abertura, onde somente os glifos ODF conseguem revelar a existência de uma região de mistura das duas direções de difusão. Vale ressaltar que a região da coroa radiada apresenta diversas configurações de cruzamento e bifurcações de duas, três ou mais fibras, \cite{schilling2017} e que muitas regiões de bifurcação de fibras ocorrem em direções oblíquas aos planos anatômicos e enfatizamos novamente que, por clareza na ilustração, os resultados mostrados dizem respeito a bifurcação de fibras cujas direções estão contidas em um plano coronal.
...
%\textcolor{blue}{
%Na região de interface entre cíngulo e corpo caloso, há a evidência de glifos que mostram as direções do corpo caloso e cíngulo analisado no experimento d
%a Seção \ref{sssec::comparacao_glifosODF_superquadrico} em todos os volumes adicionais do experimento e são facilmente identificáveis em cada um.
%%%%%%%%%%
%}
...
%\sout{Em Fig. \ref{fig::172332_crossing_odf_adicional}, observe que o cruzamento de fibra entre o SLF e o corpo caloso e cortico-espinhal é mais notável que o cruzamento entre o CC e CS. Isto evidencia o fato de que menor o ângulo de cruzamentos mais suave é a transição entre as componentes associadas a cada fibra geradas no glifo implicando em uma menor distinguibilidade na interpretação sobre as fibras subjacentes. A presença desta suavidade é característica das dODFs obtidas a partir dos métodos livres de modelo \cite{dellacqua2019}.}
